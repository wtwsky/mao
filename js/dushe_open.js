...
let key = 'dushe';
let HOST = 'https://dushe5.app';
let IMG_HOST = 'https://43.128.11.178:5001'
let parseMap = {};
let siteKey = '';
let siteType = 0;
...
async function category(tid, pg, filter, extend) {
    if (pg <= 0) pg = 1;
    const link = HOST + '/show/' + (extend.cateId || tid) + '-' + (extend.class || '') + '-' + (extend.area || '') + '-' + (extend.lang || '') + '-' + (extend.year || '') + '-' + (extend.by || '') + '-' + pg + '.html';
    const html = await request(link);
    const $ = load(html);
    const items = $('.module-item');//这一行对应每一个影视列表项，通过html搜索首个电影名来定位
    const videos = _.map(items, (item) => {
        const $item = $(item);
        const title = $item.find('.v-item-title');
        const a = $item.find('a:first');  //第一个链接项，一般情况下会包含名称和视频链接
        const img = $item.find('img:first');  //第一个图片，一般情况下就是视频封面海报
        const remarks = $item.find('.v-item-bottom').text().trim();  //备注
        return {
            vod_id: a.attr('href').replace(/\/detail\/(.*).html/g, '$1'),//视频id
            vod_name: title.text(),//名称
            vod_pic: IMG_HOST + img.attr('data-original'),//视频封面海报
            vod_remarks: remarks,//备注，对应猫影视左下角下标的内容
        };
    });
    const limit = 22;
    const hasMore = $('.pagenation-box .page-item-next').length > 0;//是否有下一页，影响猫影视的分页，一般网页里存在没有下一页的时候，下一页按钮会消失或者禁用
...
async function detail(id) {
    ...
    const vod = {
        ...
        vod_name: $('.detail-title').text().trim(),//视频名称
        vod_year: $('.detail-tags a:eq(1)').text().trim(),//年份
        vod_area: $('.detail-tags a:eq(1)').text().trim(),//地区
        vod_actor: $('.detail-info-row:contains(演员) .detail-info-row-main').text().trim().replace(/\s+\/\s+/g, '/'),//主演
        vod_director: $('.detail-info-row:contains(导演) .detail-info-row-main').text().trim().replace(/\s+\/\s+/g, '/'),//导演
        vod_pic: HOST + $('.detail-pic').attr('data-original'),//视频封面海报
        vod_remarks : $('.detail-info-row:contains(备注) .detail-info-row-main').text(),//备注，对应猫影视分类页面左下角下标的内容
        vod_content: $('.detail-desc').text().trim().replace(/\s+/g, '/'),//剧情
    };
    const playMap = {};
    /*
    线路这里稍微复杂一点，比较容易出错
    ...
    */
    const tabs = $('.source-item');//线路列表，一般包含量子、无尽等等的文案，根据网站实际的线路搜索找到
    const playlists = $('.episode-list');//每个线路对应的播放列表
    _.each(tabs, (tab, i) => {
        const $tab = $(tab);
        const from = $tab.find('span').text().trim();//线路名称
        let list = playlists[i];
        list = $(list).find('a');//线路播放列表下查找链接
        _.each(list, (it) => {
            const $it = $(it);
            let title = $it.text().trim();//线路播放列表每一项的名称
            const playUrl = $it.attr('href');//线路播放列表每一项的链接
            if (_.isEmpty(title)) title = $it.text().trim();
...
async function play(flag, id, flags) {
    const link = HOST + id;
    const html = await request(link);
    const matches = html.match(/var tcplayerOptions = {([\w\W]*?)};/);
    const tcplayerOptions = eval('let opt = {' + matches[1] + '};opt');
    const playUrl = tcplayerOptions.sources[0].src;
    return JSON.stringify({
        parse: 0,
        url: playUrl,
        header: {
...
async function search(wd, quick, pg) {
    ...
    const url = HOST + '/search?k=' + encodeURIComponent(wd) + page;
    ...
    const videos = _.map(items, (item) => {
        ...
        return {
            vod_id: $item.attr('href').replace(/\/detail\/(.*).html/g, '$1'),
            vod_name: title.text(),//名称
            vod_pic: IMG_HOST + img.attr('data-original'),
            vod_remarks: '',
        };
    });
    return JSON.stringify({
