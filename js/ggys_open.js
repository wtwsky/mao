...
async function home(filter) {
    ...
    const filterObj = {
        'movies':[{'key':'class','name':'类型','init':'','value':[{'n':'全部','v':''},{'n':'欧美电影','v':'tag/欧美电影'},{'n':'华语电影','v':'tag/华语电影'},{'n':'日韩电影','v':'tag/日韩电影'},{'n':'其他地区','v':'tag/其他地区'}]},{'key':'class','name':'','init':'','value':[{'n':'冒险','v':'genre/冒险'},{'n':'剧情','v':'genre/剧情'},{'n':'动作','v':'genre/动作'},{'n':'动画','v':'genre/动画'},{'n':'历史','v':'genre/历史'},{'n':'喜剧','v':'genre/喜剧'},{'n':'奇幻','v':'genre/奇幻'},{'n':'家庭','v':'genre/家庭'},{'n':'恐怖','v':'genre/恐怖'},{'n':'悬疑','v':'genre/悬疑'},{'n':'惊悚','v':'genre/惊悚'},{'n':'战争','v':'genre/战争'},{'n':'爱情','v':'genre/爱情'},{'n':'犯罪','v':'genre/犯罪'},{'n':'科幻','v':'genre/科幻'},{'n':'纪录','v':'genre/纪录'},{'n':'音乐','v':'genre/音乐'}]}],
        'tv-shows':[{'key':'class','name':'类型','init':'','value':[{'n':'全部','v':''},{'n':'欧美剧','v':'tag/欧美剧'},{'n':'日韩剧','v':'tag/日韩剧'},{'n':'国产剧','v':'tag/国产剧'},{'n':'其他地区','v':'tag/其他地区'}]},{'key':'class','name':'','init':'','value':[{'n':'剧情','v':'genre/剧情'},{'n':'动作','v':'genre/动作冒险'},{'n':'动画','v':'genre/动画'},{'n':'喜剧','v':'genre/喜剧'},{'n':'家庭','v':'genre/家庭'},{'n':'悬疑','v':'genre/悬疑'},{'n':'犯罪','v':'genre/犯罪'},{'n':'科幻','v':'genre/sci-fi-fantasy'},{'n':'西部','v':'genre/西部'}]}],
    };
    return {
        class: classes,
        filters: filterObj,
...
async function play(flag, id, flags) {
    ...
    const playUrl = JSON.parse(resp).video_url;
    const headers = {
        'User-Agent': UA,
        'Referer': HOST,
    };
...
async function search(wd, quick, pg) {
    ...
    const videos = [];
    const searchTypes = [TYPE_MOVIE, TYPE_TVSHOW];
    const promiseList = _.map(searchTypes, (type) => {
        const param = type.replace('-', '_');
        return request(url + param);
    });
    const respList = await Promise.all(promiseList);
    let hasMore = false;
    _.each(respList, (resp, i) => {
        try {
            const $ = load(resp);
            const prefix = searchTypes[i];
            parseVideoList($, prefix, true, videos);
            hasMore = hasMore || $('.page-numbers a.next').length > 0;
        } catch(e) {
        }
    });
    const limit = 40;
