...
async function request(reqUrl, postData, agentSp, get, cleartext) {
    let ts = dayjs().valueOf().toString();
    let rand = randStr(32);
    let sign = md5X('H58d2%gLbeingX*%D4Y8!C!!@G_' + ts + '_' + rand);
    let headers = {
    ...
    // console.log(content);
    return cleartext ? content : decrypt(content);
}

function decrypt(data) {
    const key = Crypto.enc.Utf8.parse('IjhHsCB2B5^#%0Ag');
    const iv = Crypto.enc.Utf8.parse('y8_m.3rauW/>j,}.');
...
async function play(flag, id, flags) {
    try {
        ...
        let res = await request(url + '/video.php', { url: id }, null, false, true);
...
async function search(wd, quick) {
    let data = JSON.parse(await request(url + '/api.php/Search/getSearch', { key: wd, type_id: 0, p: 1 }, null, false, true)).data;
    let videos = [];
    for (const vod of data.data) {
        videos.push({
            vod_id: vod.vod_id,
