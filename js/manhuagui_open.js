...
async function detail(id) {
    ...
    const keys = Array.from(prefixMap.keys());
    const urls = _.map(keys, (key) => {
        return _.map(list, (item) => {
            const $item = $(item);
            const title = $item.attr('title');
            const href = $item.attr('href');
            return title + '$' + href + '|' + key;
        }).join('#');
    }).join('$$$');
...
async function play(flag, id, flags) {
    try {
        ...
        const $ = load(html);
        const script = $('script:contains(RegExp)').html();
        const matches = script.match(/\[\"\\x65\\x76\\x61\\x6c\"\]\((.*)\)/);
        const js = `var result =${matches[1]};result`;
        const data = eval(js);
        const configs = data.match(/SMH\.imgData\((.*)\)\.preInit\(\);/);
        ...
        const content = _.map(json.files, (file) => {
            ...
            const encodeUrl = encodeURI(playUrl);
            return jsBase + base64Encode(encodeUrl);
        });
        return {
            content: content,
        };
