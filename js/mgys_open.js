...
async function category(tid, pg, filter, extend) {
    ...
    const $ = load(html);
    const items = $('.hl-vod-list .hl-list-item');
    let videos = _.map(items, (item) => {
        const $item = $(item);
        const it = $item.find('a:first')[0];
        const remarks = $item.find('span.remarks').text().trim();
        return {
...
async function detail(id) {
    ...
    const vod = {
        ...
        vod_lang: $('.hl-dc-content li:contains(语言：)').text().trim().substring(3),
        vod_year: $('.hl-dc-content li:contains(年份：)').text().trim().substring(3),
        vod_area: $('.hl-dc-content  li:contains(地区：)').text().trim().substring(3),
        vod_director: $('.hl-dc-content li:contains(导演：)').text().trim().substring(3).replace(/\/$/, ''),
        vod_actor: $('.hl-dc-content li:contains(主演：)').text().trim().substring(3).replace(/\/$/, ''),
        vod_pic: $('.hl-dc-content span.hl-item-thumb').attr('data-original'),
        vod_remarks : $('.hl-dc-content li:contains(状态：)').text().trim().substring(3),
        vod_content: $('.hl-dc-content li:contains(简介：)').text().trim().substring(3),
    };
    const playMap = {};
    const tabs = $('.hl-rb-playlist .hl-tabs-btn');
...
async function play(flag, id, flags) {
    ...
    if (playUrl.startsWith('Feiyun-')) {
        const parserUrl = 'https://jiexi.itcxo.cn/api/?key=V8R1PVyzcsBgG5u4b1&url=';
        const playJson = await request(parserUrl + playUrl);
        const jsConfig = JSON.parse(playJson);
        playUrl = jsConfig.url;
    } else {
        const parserUrl = 'https://player.moguys.work/player/ec.php?code=pl&if=1&url=';
        const playHtml = await request(parserUrl + playUrl);
