...
async function search(wd, quick, pg) {
    ...
    const searchUrl = HOST + '/vod-s/' + encodeURIComponent(wd) + '----------' + page + '---.html';
    ...
    const items = $('.module-list .module-search-item');
    const videos = _.map(items, (item) => {
        const $item = $(item);
        const $a = $item.find('.video-serial');
        const link = $a.attr('href')
        const title = $a.attr('title');
        const img = $item.find('.module-item-pic img:first').attr('data-src');
        const remarks = $a.text().trim();
        return {
            vod_id: link.replace(/.*?\/video\/(.*).html/g, '$1'),
            vod_name: title,
            vod_pic: img,
