...
async function home(filter) {
    let classes = [{"type_id":1,"type_name":"电影"},{"type_id":2,"type_name":"连续剧"},{"type_id":4,"type_name":"动漫"},{"type_id":3,"type_name":"综艺"},{"type_id":30,"type_name":"短剧"}];
    let filterObj = {
...
async function detail(id) {
    ...
    _.each(tabs, (tab, i) => {
        let from = tab.children[1].data.trim();
...
async function play(flag, id, flags) {
    ...
    if (parseUrl) {
        ...
        const matches = parseHtml.match(/var config = {([\w\W]*)\"time\":/);
